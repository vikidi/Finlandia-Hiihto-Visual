...
std::vector<std::vector<std::string> > InternetExplorers::FinlandiaCaller::loadData(std::shared_ptr<std::vector<InternetExplorers::FinlandiaAPI::Parameters> > searchVector)
{
    ...
    textPart1.setBody(search.year.toLatin1());
    ...
    textPart2.setBody(search.trip.toLatin1());
    ...
    textPart3.setBody(search.gender.toLatin1());
    ...
    textPart4.setBody(search.age.toLatin1());
    ...
    textPart5.setBody(search.firstName.toLatin1());
    ...
    textPart7.setBody(search.nationality.toLatin1());
    ...
    connect(m_reply, QOverload<QNetworkReply::NetworkError>::of(&QNetworkReply::error), &loop, &QEventLoop::quit);

    timer.start(200000);   // 200 secs. timeout
    loop.exec();
    ...
    if(timer.isActive()) {
        ...
        if(m_reply->error() > 0) { // Error
            ...
            if (v >= 200 && v < 300) {  // Success
                ...
                std::vector<std::vector<std::string>> pdata = parseData(data);
                return pdata;
            }
        }
    } else {
...
std::vector<std::vector<std::string> > InternetExplorers::FinlandiaCaller::parseData(const std::string &src)
{
    ...
    std::size_t ind = src.find("id=\"dnn_ctr1025_Etusivu_dgrTulokset_ctl00\"");
    std::size_t startind = src.find(">", ind + 1);
    std::size_t endind = src.find("</table", startind + 1);

    // Get the result table as string
    std::string stringdata = src.substr(startind + 1, endind - startind - 1);
